...
import { route, authorized } from './api/routes.js'
...
const server = http.createServer(async (req, res) => {
  ...
  if (!(path === '/health' && req.method === 'GET') && !authorized(req)) {
  ...
  try {
    ...
    const headers = result.contentType
      ? { ...defaultHeaders, 'Content-Type': result.contentType }
      : defaultHeaders
...
async function start() {
  if (!config.apiToken) {
    console.warn('[server] WARNING: API_TOKEN is empty, auth disabled')
  }
