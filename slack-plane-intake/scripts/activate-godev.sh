...
ssh "$host" 'set -euo pipefail; release="$HOME/.local/share/slack-plane-intake/current"; unit_dir="$HOME/.config/systemd/user"; dropin_dir="$unit_dir/hermes-gateway.service.d"; install -d -m 0700 "$dropin_dir"; if [[ ! -f "$unit_dir/hermes-gateway.service" ]]; then install -m 0644 "$release/deploy/hermes-gateway.service" "$unit_dir/hermes-gateway.service"; fi; install -m 0644 "$release/deploy/hermes-gateway-hardening.conf" "$dropin_dir/10-cslog-179-hardening.conf"; systemctl --user daemon-reload; systemctl --user enable hermes-gateway.service >/dev/null; systemctl --user restart hermes-gateway.service; for attempt in 1 2 3 4 5; do systemctl --user is-active --quiet hermes-gateway.service && break; sleep 1; done; systemctl --user is-active hermes-gateway.service'
