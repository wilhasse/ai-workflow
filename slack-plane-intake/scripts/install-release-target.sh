...
"$staging_dir/venv/bin/pip" install "${staging_dir}[dev]"
"$staging_dir/venv/bin/ruff" format --check "$staging_dir"
"$staging_dir/venv/bin/ruff" check "$staging_dir"
"$staging_dir/venv/bin/pytest" -q "$staging_dir/tests"
